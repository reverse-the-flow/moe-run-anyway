#!/usr/bin/env python3
"""Shared Phase 3 candidate router-trace receipt helpers."""
...
def default_trace_receipt(
    *,
    prompt_set: JSONDict,
    source_prompt_set_path: str | None,
    candidate_trace_path: str | None,
) -> JSONDict:
    return {
        "receipt_ready": False,
        ...
        "operator_notes": "fill after approved router trace capture",
    }


def validate_trace_receipt(receipt: Any) -> JSONDict:
    errors: list[str] = []
    if not isinstance(receipt, dict):
        return {
            "shape_valid": False,
            "receipt_ready": False,
            "errors": ["trace_capture_receipt must be an object"],
        }

    ready_value = receipt.get("receipt_ready")
    if not isinstance(ready_value, bool):
        errors.append("trace_capture_receipt.receipt_ready must be a boolean")
        receipt_ready = False
    else:
    ...
    if receipt_ready:
        for field in READY_TEXT_FIELDS:
            if string_or_none(receipt.get(field)) is None:
                errors.append(f"trace_capture_receipt.{field} must be a non-empty string when receipt_ready is true")
        for field in READY_BOOLEAN_FIELDS:
            if receipt.get(field) is not True:
                errors.append(f"trace_capture_receipt.{field} must be true when receipt_ready is true")
...
def validate_trace_receipt_source_binding(
    receipt: Any,
    *,
    repo_root: Path,
    expected_source_request_path: Path | None = None,
    expected_source_prompt_set_path: Path | None = None,
    expected_candidate_trace_path: Path | None = None,
    require_existing_sources: bool = True,
) -> JSONDict:
    validation = validate_trace_receipt(receipt)
    ...
    fields = {
        ...
        "candidate_trace_path": expected_candidate_trace_path,
    }
    ...
    if isinstance(receipt, dict):
        for field, expected_path in fields.items():
            ...
            if expected_path is not None and value_present and not matches[field]:
                errors.append(f"trace_capture_receipt.{field} must match expected {field}")
            if receipt_ready and require_existing_sources and not exists[field]:
                errors.append(f"trace_capture_receipt.{field} must point to an existing file when receipt_ready is true")
